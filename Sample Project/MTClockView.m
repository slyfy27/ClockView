...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i = 0; i<100; i++) {
        ...
        CGPoint P1 = CGPointMake((self.frame.size.width/2 + ((self.frame.size.width - self.borderWidth*2 - graduationOffset) / 2) * cos((3.6*i)*(M_PI/180)  - (M_PI/2))), (self.frame.size.width/2 + ((self.frame.size.width - self.borderWidth*2 - graduationOffset) / 2) * sin((3.6*i)*(M_PI/180)  - (M_PI/2))));
        CGPoint P2 = CGPointMake((self.frame.size.width/2 + ((self.frame.size.width - self.borderWidth*2 - graduationOffset - self.graduationLength) / 2) * cos((3.6*i)*(M_PI/180)  - (M_PI/2))), (self.frame.size.width/2 + ((self.frame.size.width - self.borderWidth*2 - graduationOffset - self.graduationLength) / 2) * sin((3.6*i)*(M_PI/180)  - (M_PI/2))));
    ...
    for(int i = 0; i < 10; i ++){
        ...
        [hourNumber drawInRect:CGRectMake(labelX - digitFont.lineHeight/2.0f,labelY - digitFont.lineHeight/2.0f,30,digitFont.lineHeight) withAttributes:@{NSForegroundColorAttributeName: [UIColor blackColor], NSFontAttributeName: digitFont}];
    }
}
